...
import { fetchTopProductAddo, fetchTopProductSupervisor } from '@/app/lib/data';
...
export default async function topSoldTable({
    ...
    if (user?.roles === 'Supervisor') {
        if (startDate && endDate && region && district) {
            const products = await fetchTopProductSupervisor(region, district, startDate, endDate);
            return (
