...
import { getUser, fetchSupplier, fetchProductType, fetchMedicalType, fetchSellingUnit, fetchProductStockAddoById, fetchMasterListById } from "@/app/lib/data";
import Link from "next/link";
import { useEffect, useState } from "react";
import { Button } from "../button";
import { useFormState } from "react-dom";
import notFound from "@/app/dashboard/stock-management/[id]/edit/not-found";
import { updateMasterList, updateProductStock } from "@/app/lib/actions";
...
export default function Form({ id }: { id: string }) {
  {
    ...
    const updateMasterListId = updateMasterList.bind(null, masterList.id)
    const [Estate, Edispatch] = useFormState(updateMasterListId, initialState);
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          const masterList = await fetchMasterListById(id)
          if (!masterList) {
            notFound();
          } else {
    ...
    if (user?.roles === 'Owner') {
      ...
      return (
        <form action={Edispatch}>
          <div className="rounded-md bg-gray-50 p-4 md:p-6">
            <div className="mb-4">
              <label htmlFor="dosageType" className="mb-2 block text-sm font-medium">
                Choose dosage type
              </label>
              <div className="relative">
                <select
                  ...
                  name="dosageType"
                  className="peer block w-full cursor-pointer rounded-md border border-gray-200 py-2 pl-10 text-sm outline-2 placeholder:text-gray-500"
                  defaultValue={masterList?.medicineType}
                  aria-describedby="medicineType-error"
                >
                  <option value="" disabled>
                    Select a dosage type
                  </option>
                  {medicalType.map((data) => (
                    <option key={data.id} value={data.id}>
                      {data.key}
              ...
              <div id="dosageType-error" aria-live="polite" aria-atomic="true">
                {Estate.errors?.medicineType &&
                  Estate.errors.medicineType.map((error: string) => (
                    <p className="mt-2 text-sm text-red-500" key={error}>
                      {error}
                    </p>
                  ))}
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                <input
                  ...
                  id="genericname"
                  name="genericname"
                  defaultValue={masterList?.genericName}
                  className="peer block w-full cursor-pointer rounded-md border border-gray-200 py-2 pl-10 text-sm outline-2 placeholder:text-gray-500"
                  placeholder="Enter generic name"
                  aria-describedby="genericname-error"
                // readOnly={true}
              ...
              <div id="genericname-error" aria-live="polite" aria-atomic="true">
                {Estate.errors?.genericName &&
                  Estate.errors.genericName.map((error: string) => (
                    <p className="mt-2 text-sm text-red-500" key={error}>
                      {error}
                    </p>
                  ))}
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                <input
                  ...
                  id="brandname"
                  name="brandname"
                  defaultValue={masterList?.brandName}
                  className="peer block w-full cursor-pointer rounded-md border border-gray-200 py-2 pl-10 text-sm outline-2 placeholder:text-gray-500"
                  placeholder="Enter brand name"
                  aria-describedby="brandname-error"
                // readOnly={true}
              ...
              <div id="brandname-error" aria-live="polite" aria-atomic="true">
                {Estate.errors?.brandName &&
                  Estate.errors.brandName.map((error: string) => (
                    <p className="mt-2 text-sm text-red-500" key={error}>
                      {error}
                    </p>
                  ))}
            ...
            <div className="mb-4">
              <label htmlFor="MedicinalStrength" className="mb-2 block text-sm font-medium">
                Medicine Strength
                <span className="text-red-500">*</span>
              </label>
              <div className="relative">
                <input
                  type="text"
                  id="MedicinalStrength"
                  name="MedicinalStrength"
                  defaultValue={masterList?.medicineStrength}
                  className="peer block w-full cursor-pointer rounded-md border border-gray-200 py-2 pl-10 text-sm outline-2 placeholder:text-gray-500"
                  placeholder="Enter Medicine strength"
                  aria-describedby="MedicinalStrength-error"
                // readOnly={true}
                />

              </div>
              <div id="MedicinalStrength-error" aria-live="polite" aria-atomic="true">
                {Estate.errors?.MedicineStrength &&
                  Estate.errors.MedicineStrength.map((error: string) => (
                    <p className="mt-2 text-sm text-red-500" key={error}>
                      {error}
                    </p>
                  ))}
